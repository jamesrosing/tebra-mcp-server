...
6. **Single-record ops have different contracts from their list counterparts.** `GetPatientReq` is Filter-only (SinglePatientFilter — no Fields member at all); `GetAppointmentReq` takes `<Appointment>{AppointmentId}`; `GetAppointmentReasonsReq` takes a bare `PracticeId` — REQUIRED (verified live: omitting it faults; the handler auto-resolves it via GetPractices); `GetCustomerIdFromKeyRequest` takes bare CustomerKey/Password/User (it does not extend RequestBase). Never assume the Fields/Filter shape without reading the Req type.

7. **xsd7 types live in a DIFFERENT XML namespace — no trailing slash.** xsd0's targetNamespace is `http://www.kareo.com/api/schemas/`; xsd7's (GetServiceLocations, GetProcedureCodes) is `http://www.kareo.com/api/schemas` (no slash). Members of xsd7 request types (Fields/Filter/criteria) must be emitted with the `kar7:` prefix declared in `buildEnvelope()`, while the `<kar:request>` wrapper and RequestHeader stay in the xsd0 namespace. Wrong namespace → WCF skips the elements and faults "Expecting element 'Fields'". Verified live 2026-08-03.
...
**Useful enum facts (from the WSDL)**: `EncounterStatusCode` = Draft | Submitted | Approved | Rejected | Unpayable — there is NO 'Review' status; Tebra's UI shows Submitted encounters under "Review". GetEncounterDetails returns EncounterStatus as a 1-based NUMERIC code (3=Approved verified live; mapped in encounters.ts). `AppointmentStatus` has 13 values (Scheduled, Confirmed, CheckedIn, Roomed, ReadyToBeSeen, CheckedOut, NoShow, Cancelled, ...). `GenderCode` = Male | Female | Unknown (no 'Other'). `Relationship` (guarantor) = Child | Other | Self | Spouse. `AppointmentFilter` has no ProviderID member (filter by ResourceName); `PaymentFilter` has no PatientID; `PatientFilter` has no MRN/ExternalID/exact-DOB (use `FromDateOfBirth`/`ToDateOfBirth`, capital O — and `ToDateOfBirth` is EXCLUSIVE: an exact-DOB search must send [DOB, DOB+1), verified live).

**Live smoke status (2026-08-04)**: all read tools + FHIR pipeline verified against production (24/24), AND the write path verified via the EPIC-Notes harness — CreatePatient, CreateAppointment, CreateDocument, and the external-ID batch + vendor-scoped lookup all succeeded end-to-end. Unverified live: CreateEncounter, UpdateEncounterStatus, UpdatePatient demographics, UpdateAppointment (billing-affecting; verify against a draft encounter when safe). Tebra's 1000ms per-endpoint window needs a client-side margin (+250ms in `enforceRateLimit`) — an exact-interval gap still 429s. The FHIR token server rejects a wrong scope with an error body that names the registered scope; `fhir-client.ts` auto-retries with it (this account's registration is `patient/*.read`, not `system/*.read`).

**Live-verified write facts (2026-08-04)**: required members per the minOccurs audit — PatientCreate/Update need `Practice`, AppointmentCreate needs `PracticeId`, AppointmentUpdate needs `PatientId`+`ServiceLocationId`, DocumentCreateRequest needs `PracticeId` (handlers auto-resolve via cached GetPractices / hydrate via GetAppointment). External IDs: vendor-scoped (`ExternalIDToPatientMap`), UNIQUE per vendor, **silently truncated at 25 chars** — clients fail closed above 25; the only reliable write path is `UpdatePatientsExternalID` (pushing `PatientExternalID` through UpdatePatient 500s); the response's `ItemsUpdated` echo is EMPTY even on success — verify via the vendor-scoped `ExternalID` lookup on GetPatient. True `xs:dateTime` members (DocumentDate, StartTime/EndTime, DateofBirth) need ISO 8601 — the MM/dd/yyyy form that the `xs:string` Filter members accept faults the deserializer.
