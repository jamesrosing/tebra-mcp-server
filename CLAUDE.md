...
**Live smoke status (2026-08-04)**: all read tools + FHIR pipeline verified against production (24/24 pass — see `memory/project_tebra_040_release.md`); write tools remain WSDL-derived + test-pinned only. Tebra's 1000ms per-endpoint window needs a client-side margin (+250ms in `enforceRateLimit`) — an exact-interval gap still 429s. The FHIR token server rejects a wrong scope with an error body that names the registered scope; `fhir-client.ts` auto-retries with it (this account's registration is `patient/*.read`, not `system/*.read`).
